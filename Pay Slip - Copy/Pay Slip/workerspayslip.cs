            ...
            using (SqlConnection tblOTS1 = new SqlConnection(ConfigurationManager.ConnectionStrings["otspayroll"].ConnectionString))
            {
                ...
                if (UserExist > 0)
                {
                    ...
                    using (SqlConnection tblOTS = new SqlConnection(ConfigurationManager.ConnectionStrings["otspayroll"].ConnectionString))
                    {
                        ...
                        tblOTS.Open();
                        DataTable dt = new DataTable();
                        using (SqlDataAdapter a2 = new SqlDataAdapter("SELECT * FROM LEOPAY WHERE PERIOD = '" + fdate + "'", tblOTS))
                            a2.Fill(dt);
                    ...
                    foreach (DataGridViewRow row in dataGridView1.Rows)
                    {
                        using (SqlConnection tblOTS = new SqlConnection(ConfigurationManager.ConnectionStrings["otspayroll"].ConnectionString))
                        {
                            ...
                            tblOTS.Open();
                            String query3 = "SELECT ACCTDESC FROM GLU4 WHERE ACCTCODE = '" + row.Cells[13].Value + "'";
                            ...
                            insCmd2.Parameters.AddWithValue("@ACCTCODE", row.Cells[13].Value.ToString());
            ...
            try
            {
                ...
                using (SqlConnection tblOTS = new SqlConnection(ConfigurationManager.ConnectionStrings["otspayroll"].ConnectionString))
                {
                    ...
                    else
                    {
                        ...
                        new System.Threading.Thread(new System.Threading.ThreadStart(submit));
                        thread.Start();
                    }
                    tblOTS.Close();
                }
