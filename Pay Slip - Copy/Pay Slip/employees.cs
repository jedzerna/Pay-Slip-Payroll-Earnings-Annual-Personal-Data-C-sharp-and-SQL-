...
namespace Pay_Slip
{
    public partial class employees : Form
    {
        ...
        {
            (dataGridView1.DataSource as DataTable).DefaultView.RowFilter = string.Format("FIRST LIKE '%" + guna2TextBox2.Text.Replace("[", lb).Replace("]", rb).Replace("​*", "[*​]").Replace("%", "[%]").Replace("'", "''") + "%' OR FAMILY LIKE '%" + guna2TextBox2.Text.Replace("[", lb).Replace("]", rb).Replace("​*", "[*​]").Replace("%", "[%]").Replace("'", "''") + "%'");
        }
        public string type;
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (access == true)
            {
                ...
                P.EMPN = dataGridView1.CurrentRow.Cells["Column4"].Value.ToString().Trim() + ", " + dataGridView1.CurrentRow.Cells["Column2"].Value.ToString().Trim() + " " + dataGridView1.CurrentRow.Cells["Column3"].Value.ToString().Trim();
                P.ShowDialog();
            }
            else
            {
