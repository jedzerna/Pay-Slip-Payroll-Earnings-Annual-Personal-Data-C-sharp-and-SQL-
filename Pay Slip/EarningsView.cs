...
namespace Pay_Slip
{
    public partial class EarningsView : Form
    {
        ...
        public string id;
        private void EarningsView_Load(object sender, EventArgs e)
        {
            //genarate(); 
        ...
        private void EarningsView_FormClosing(object sender, FormClosingEventArgs e)
        {

        }

        private void EarningsView_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Hide();
        ...
        private void genarate()
        {
            ...
            using (SqlConnection tblOTS = new SqlConnection(ConfigurationManager.ConnectionStrings["otspayroll"].ConnectionString))
            {
                ...
                tblOTS.Open();
                string list = "SELECT EMPCODE,EMPLOYEENAME,PERIOD FROM tblFinalOTS WHERE PERIOD BETWEEN @date1 AND @date2 ORDER BY PERIOD ASC";
                ...
                foreach (DataRow item in distinct.Rows)
                {
                    ...
                    if (checking == false)
                    {
                        ...
                        dataGridView1.Rows[a].Cells["NAME"].Value = item["EMPLOYEENAME"].ToString();
                    }
                }
        ...
        private void getdata()
        {
            ...
            using (SqlConnection tblOTS = new SqlConnection(ConfigurationManager.ConnectionStrings["otspayroll"].ConnectionString))
            {
                ...
                DataTable distinct = new DataTable();
                tblOTS.Open();
                string list = "SELECT DISTINCT PERIOD FROM tblFinalOTS WHERE PERIOD BETWEEN @date1 AND @date2 ORDER BY PERIOD ASC";
                SqlDataAdapter command = new SqlDataAdapter(list, tblOTS);
                command.SelectCommand.Parameters.AddWithValue("@date1", from);
                command.SelectCommand.Parameters.AddWithValue("@date2", to);
                ...
                foreach (DataRow item in distinct.Rows)
                {
                    ...
                    {
                        if (DateTime.Parse(item["PERIOD"].ToString()).ToString("MM/yyyy") == col.Name)
                        {
                            check = true;
                        }
                    }
                    if (check == false)
                    {
                        dataGridView2.Columns.Add(DateTime.Parse(item["PERIOD"].ToString()).ToString("MM/yyyy"), DateTime.Parse(item["PERIOD"].ToString()).ToString("MM/yyyy"));
                        dataGridView3.Columns.Add(DateTime.Parse(item["PERIOD"].ToString()).ToString("MM/yyyy"), DateTime.Parse(item["PERIOD"].ToString()).ToString("MM/yyyy"));
                        int year = Convert.ToInt32(DateTime.Parse(item["PERIOD"].ToString()).ToString("yyyy"));
                        int month = Convert.ToInt32(DateTime.Parse(item["PERIOD"].ToString()).ToString("MM"));
                        ...
                        object[] w = { DateTime.Parse(item["PERIOD"].ToString()).ToString("MM/yyyy") };
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                ...
                using (SqlConnection tblOTS = new SqlConnection(ConfigurationManager.ConnectionStrings["otspayroll"].ConnectionString))
                {
                    ...
                    foreach (DataRow rows in listofmonths.Rows)
                    {
                        ...
                        tblOTS.Open();
                        string list = "SELECT EMPCODE,PERIOD,DAYRATE,NUMDAY FROM tblFinalOTS WHERE EMPCODE = '" + row.Cells["EMPCODE"].Value.ToString() + "' AND PERIOD BETWEEN @date1 AND @date2";
                        SqlDataAdapter command = new SqlDataAdapter(list, tblOTS);
                        ...
                        {
                            decimal sum = 0.00M;
                            sum += Convert.ToDecimal(datarow["DAYRATE"].ToString()) * Convert.ToDecimal(datarow["NUMDAY"].ToString());
                            sumpermonths += sum;
                        }
            ...
            this.dataGridView2.Columns["dataGridViewTextBoxColumn2"].Width = 250;
        ...
        {
            if (guna2Button2.Text == "Exclude")
            {
                dataGridView2.Columns["exclude"].Visible = true;
                guna2Button2.Text = "Cencel";
            }
            else
            {
                dataGridView2.Columns["exclude"].Visible = false;
                guna2Button2.Text = "Exclude";
            }
        }
