            ...
            if (File.Exists(path) && File.Exists(path2) && File.Exists(path3))
            {
                ...
                using (SqlConnection tblOTS = new SqlConnection(ConfigurationManager.ConnectionStrings["otspayroll"].ConnectionString))
                {
                    ...
                    foreach (DataGridViewRow row in dataGridView1.Rows)
                    {
                        ...
                        tblOTS.Open();
                        String querSy2 = "SELECT * FROM tblEMPCon WHERE EMPCODE = '" + row.Cells["empcode"].Value.ToString().Trim() + "'";
                        SqlCommand cmAd2 = new SqlCommand(querSy2, tblOTS);
                        SqlDataReader rdrX2 = cmAd2.ExecuteReader();

                        if (rdrX2.Read())
                        {
                            if (rdrX2["HDMFONOFF"].ToString() == "TRUE")
                            {
                                row.Cells["Column8"].Value = rdrX2["HDMF"].ToString();
                                //MessageBox.Show(rdrX2["HDMF"].ToString());
                            }
                            else
                            {
                                row.Cells["Column8"].Value = "0.00";
                            }
                        }
                        else
                        {
                        ...
                        tblOTS.Open();
                        String query2 = "SELECT PERIODD,ADV,COOP,SSSLOAN,OTHERS FROM tblOTSDEDUC WHERE EMPCODE = '" + row.Cells[9].Value + "' AND PERIODD = '" + row.Cells[11].Value + "'";
                        ...
                        if (rdr2.Read())
                        {
                            ...
                            //row.Cells["Column8"].Value = rdr2["HDMFCONT"].ToString();
                        ...
                        string totaldeduc;
                        decimal deduc = 0.00M;
                        deduc += Convert.ToDecimal(row.Cells[3].Value.ToString()) + Convert.ToDecimal(row.Cells[4].Value.ToString()) + Convert.ToDecimal(row.Cells[5].Value.ToString()) + Convert.ToDecimal(row.Cells[6].Value.ToString()) + Convert.ToDecimal(row.Cells[18].Value.ToString()) + Convert.ToDecimal(row.Cells[7].Value.ToString());
